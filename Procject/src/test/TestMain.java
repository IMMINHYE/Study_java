package test;

import java.io.FileOutputStream;
import java.io.IOException;
import java.io.OutputStreamWriter;

import school.School;
import school.Score;
import school.Student;
import school.Subject;
import school.report.GenerateGradeReport;
import utils.Define;

public class TestMain {

		School goodSchool = School.getInstance();
		Subject korean;
		Subject math;
		Subject dance;
		GenerateGradeReport gradeReport = new GenerateGradeReport();
		
		public static void main(String[] args) {
			TestMain test = new TestMain();
			
			test.creatSubject();
			test.creatStudent();
			
			String report = test.gradeReport.getReport();
			System.out.println(report);
			try(FileOutputStream fos = new FileOutputStream("resultinfo.txt");
					OutputStreamWriter osw = new OutputStreamWriter(fos)){
						osw.write(report);
					} catch (IOException e) {
						System.out.println(e);
					}
					System.out.println("studentinfo.txt 파일을 열어보세요");
		}
		
		public void creatSubject() {
			korean = new Subject("국어", Define.KOREAN);
			math = new Subject("수학", Define.MATH);
			dance = new Subject("방송댄스", Define.DANCE);
			
			goodSchool.addSubject(korean);
			goodSchool.addSubject(math);
			goodSchool.addSubject(dance);
		}
		
		public void creatStudent() {
			Student student1 = new Student(220511, "이상용", math);
			Student student2 = new Student(181213, "안성원", korean);
			Student student3 = new Student(181518, "오태훈", math);
			Student student4 = new Student(171230, "이동호", korean);
			Student student5 = new Student(171255, "조성욱", korean);
			Student student6 = new Student(171590, "최태평", math);
			
			goodSchool.addStudent(student1);
			goodSchool.addStudent(student2);
			goodSchool.addStudent(student3);
			goodSchool.addStudent(student4);
			goodSchool.addStudent(student5);
			goodSchool.addStudent(student6);
			
			korean.register(student1);
			korean.register(student2);
			korean.register(student3);
			korean.register(student4);
			korean.register(student5);
			korean.register(student6);
			
			math.register(student1);
			math.register(student2);
			math.register(student3);
			math.register(student4);
			math.register(student5);
			math.register(student6);
			
			dance.register(student1);
			dance.register(student2);
			dance.register(student3);
			dance.register(student4);
			dance.register(student5);
			dance.register(student6);
			
			addScoreForStudent(student1, korean, 100);
			addScoreForStudent(student1, math, 100);
			addScoreForStudent(student1, dance, 100);
			
			addScoreForStudent(student2, korean, 95);
			addScoreForStudent(student2, math, 56);
			addScoreForStudent(student2, dance, 95);

			addScoreForStudent(student3, korean, 95);
			addScoreForStudent(student3, math, 95);
			addScoreForStudent(student3, dance, 85);

			addScoreForStudent(student4, korean, 100);
			addScoreForStudent(student4, math, 88);
			addScoreForStudent(student4, dance, 55);

			addScoreForStudent(student5, korean, 89);
			addScoreForStudent(student5, math, 95);

			addScoreForStudent(student6, korean, 85);
			addScoreForStudent(student6, math, 55);
			
		}

		public void addScoreForStudent(Student student, Subject subject, int point) {
			Score score = new Score(student.getStudentId(), subject, point);
			student.addSubjectScore(score);
		}
}
